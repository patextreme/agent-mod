...
- **Relative-only import policy**: every import specifier in a flow's import graph must start with `./` or `../`. Bare/npm specifiers and `node:` builtins are rejected. Relative paths may escape the flow directory (no confinement). CJS `require("./x")` edges are tolerated and walked; the former module-syntax guard is removed. Unverifiable loader entry points — `module.require`, `module.createRequire`, and `eval()` — are rejected.
