...
- **Implementation wiring.** The SDK's `getShellConfig`/`killProcessTree` helpers are injected via `RunnerServices` (same pattern as `spawnSession`); a new SDK-free `exec.ts` does spawn/collect; `FlowRunner` owns the active-children registry and kill-on-cancel.
...
- **Code**: `extensions/agentflow/` — `agentflow.d.ts` (types), new `exec.ts` (SDK-free spawn/collect), `runner.ts` (`buildAf` wiring, kill registry, failure notice), `runtime.ts` (re-export surface), `index.ts` (inject SDK helpers via `RunnerServices`).
