...
- [x] 1.1 In `extensions/permission/index.ts`, add module-level `yoloEnabled` state plus `setYoloStatus`, `enableYolo` (no-UI refusal + confirm dialog), and `disableYolo` helpers per design.md D1–D3, D5
...
- [x] 3.2 Manual smoke test in pi TUI: toggle on (confirm shown), verify `⚠️ YOLO MODE ON` in warning color, run a command that would prompt (e.g. `git push --dry-run`) and confirm it runs unasked, toggle off, verify warning gone and prompt behavior restored; verify invalid arg errors
