...
- Expose the TypeBox `Type` namespace on the `af` surface (`af.Type`) so flow scripts can construct a `resultSchema` without importing (flow scripts cannot `import`; `af` is their only global).
...
- `agentflow-authoring`: extends the shipped `agentflow.d.ts` declarations (generic `FlowAgent<T>`, `resultSchema` config, `submittedResult`, `clearResult`, and `af.Type`) and the authoring skill with the new surface and orchestration patterns.

## Impact

- `extensions/agentflow/runtime.ts` — `FlowAgentHandle` gains the submission slot, `submittedResult()`, `clearResult()`; `createAgent` builds and injects the `submit_result` tool via `customTools` when `resultSchema` is present; `buildAf` exposes the typebox `Type` namespace as `af.Type`.
- `extensions/agentflow/agentflow.d.ts` — new type surface; imports `TSchema` and `Type` from `typebox`; declares `Type: typeof Type` on the `AgentFlow` interface.
